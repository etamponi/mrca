...
class TestManualCentroidCluster(unittest.TestCase):
    ...
    def test_centroid_sorted_by_mri(self):
        ...
        cluster.fit(inputs)
        self.assertEqual((5, 20), cluster.cluster_centers_.shape)
        self.assertTrue(numpy.all(numpy.logical_and(-1 <= cluster.cluster_centers_, cluster.cluster_centers_ <= 1)))
        # Should be sorted in decreasing mri order
        mris = MRI()(cluster.cluster_centers_)
